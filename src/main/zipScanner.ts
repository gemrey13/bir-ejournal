...
function parseInnerZip(name: string): { month: number; year: number } | null {
  ...
  const match = path.basename(name, path.extname(name)).toUpperCase().match(/^OR(\d{4})(\d{2})$/);
  ...
  try {
    for (const yearFolder of fs.readdirSync(branchPath)) {
      ...
      for (const outerName of fs.readdirSync(yearPath)) {
        ...
        let outerZip: AdmZip;
        try { outerZip = new AdmZip(path.join(yearPath, outerName)); }
        catch { continue; }
        ...
        for (const entry of outerZip.getEntries()) {
          ...
          let innerZip: AdmZip;
          try { innerZip = new AdmZip(tmpZip); }
          catch { fs.unlinkSync(tmpZip); continue; }
